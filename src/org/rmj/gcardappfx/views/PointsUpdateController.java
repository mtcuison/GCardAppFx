...
import org.rmj.gcard.trans.agentFX.XMGCUpdate;

public class PointsUpdateController implements Initializable {

    @FXML
    private TableView tblUnposted;
    @FXML
    private TextField txtSeeks00;
    @FXML
    ...
    private final String pxeModuleName = "G-Card Points Update";
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        poTrans = new XMGCUpdate(poGRider, psBranchCd , false);
    ...
    public void loadGui(){
        ...
        mainController.cmdButton02.setText("UPDATE");
        mainController.cmdButton03.setText("BACK");
    ...
    private void cmdButton_Click(ActionEvent event) {
        ...
        switch (lsButton){
            ...
            case "cmdbutton01":
                ...
                break;
            case "cmdbutton02":
                if (poTrans.getMaster("sCardNmbr") == null ||poTrans.getMaster("sCardNmbr").equals("")){
                    ShowMessageFX.Warning(null, pxeModuleName, "No loaded data!");
                    return;
                }
                
                if(ShowMessageFX.OkayCancel(null, pxeModuleName, "Do you want to update points of this card?")==true){
                    boolean lbResult; 
                    if (poTrans.getMaster("cMainGrpx").toString().equals("1"))
                        lbResult = poTrans.updateMaster();
                    else
                        lbResult = poTrans.saveUpdate();

                    if(lbResult){
                        //disconnect the card
                        poTrans.releaseCard();
                        
                        ShowMessageFX.Information(null, pxeModuleName, "Successfully updated!");
                        clearFields();
                    }else{
                        ShowMessageFX.Warning(null, pxeModuleName, poTrans.getMessage());
                    }  
                ...
                break;
            case "cmdbutton03":
                mainController.loadModule(pxePointsInquiry);
                break;  
            case "cmdbutton04":
            case "cmdbutton05":
            case "cmdbutton06":
    ...
    public void loadTransaction(){
        ...
        if (loDevice != null){
            ...
            txtField03.setText((String) poTrans.getBranch((String) loDevice.getCardInfo("sGCardNox").toString().substring(0, 4)).getBranchNm());
            txtField06.setText(CommonUtils.NumberFormat(Double.valueOf(loDevice.getCardInfo("nTotPoint").toString()), "0.0"));
            ...
            txtSeeks00.setText((String) poTrans.getMaster("sCardNmbr").toString().replaceFirst("(\\w{5})(\\w{6})(\\w+)", "$1-$2-$3"));
            txtField00.setText((String) poTrans.getMaster("sCardNmbr").toString().replaceFirst("(\\w{5})(\\w{6})(\\w+)", "$1-$2-$3"));
            txtField03.setText((String) poTrans.getBranch((String) poTrans.getMaster("sGCardNox").toString().substring(0, 4)).getBranchNm());
            txtField06.setText(CommonUtils.NumberFormat(Double.valueOf(poTrans.getMaster("nTotPoint").toString()), "0.0"));
            ...
            txtField05.setText(CommonUtils.NumberFormat(Double.valueOf(poTrans.getMaster("nAvlPoint").toString()), "0.0"));
        }
        ...
        if (poTrans.getMaster("dActivate") == null){
            txtField04.setText(CommonUtils.xsDateMedium(CommonUtils.toDate(pxeCurrentDate)));
        }else{
            txtField04.setText(CommonUtils.xsDateMedium((Date) poTrans.getMaster("dActivate")));
        }
        
        if (poTrans.getMaster("dMemberxx") ==null){
            txtField07.setText(CommonUtils.xsDateMedium(CommonUtils.toDate(pxeCurrentDate)));
        }else{
    ...
    public void loadOfflinePoints(){
        int lnCtr;
        int lnRow = poTrans.getSize();
        double total = 0.0;
        
        if (lnRow == 1 && poTrans.getDetail(0, "sTransNox").toString().equals("")){
            txtField09.setText("0.0");
            initGridView();
            return;
        }
        ...
        txtField09.setText(CommonUtils.NumberFormat(Double.valueOf(total) , "0.0"));
